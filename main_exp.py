...
def main():
    ...
    for subspace_id in range(subspace_num):
        tmp_SS_embeddings = SS_embeddings_list[subspace_id]
        subspace_vocab = SS_word_list[subspace_id]
        ######################################################################################
        # CS(with class)特徴量の計算
        cos_sim = cosine_similarity(
            tmp_SS_embeddings, query_embeddings
        )  # tmp_ss_embedding, shape(subspace内の単語数,768)
        cos_sim = cos_sim.mean(axis=1)
        cos_sim = cos_sim.mean(axis=0)
        cs_class_old = round(cos_sim, 3)
        ######################################################################################
        # GS特徴量の計算
        gs_value = []
        for class_idx in range(query_embeddings.shape[0]):  # クラス数分回す
            A = np.array(tmp_SS_embeddings)  # [単語数,768]
            Q = schmidt(A)  # [単語数,768],Qには各keywordのembeddingであるwが格納
            f_v = np.zeros(Q.shape[1])

            for idx in range(Q.shape[0]):  # 単語数分回す
                f_v += np.dot(query_embeddings[class_idx, :], Q[idx, :]) * Q[idx, :]

            gs_value.append(np.linalg.norm(query_embeddings[class_idx, :] - f_v))
        gs_value = round(sum(gs_value) / len(gs_value), 3)
        ######################################################################################
        ...
        ######################################################################################
        # for subspace_id in range(subspace_num):  # 各subspaceを取得
        # Calibration
        if args.dataset == "waterbirds":
            text_descriptions = [
                "This is a picture of a landbird.",
        ...
        with open(
            ...
            writer = csv.writer(f)
            writer.writerow(
                [V, subspace_num, adj_avg_acc, robust_acc, gap, gs_value, cs_class_old]
            )
    ######################################################################################
