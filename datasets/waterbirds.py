...
class Waterbirds(Dataset):
    ...
    def __init__(
        ...
        self.data_dir = os.path.join(
            self.root_dir, "_".join([self.target_name] + self.confounder_names)
        )
...
class Waterbirds_fail(Dataset):
    ...
    def __init__(
        ...
        self.metadata_df = pd.read_csv(
            "/nas/data/ryota23/DATA/data/Waterbirds/metadata_train_incorrect.csv"
        )
        # Filter for data split ('train', 'val', 'test')
        self.metadata_df = self.metadata_df[
            self.metadata_df["split"] == self.split_dict[self.split]
...
def load_waterbirds(args, train_shuffle=True, transform=None):
    ...
    train_loader = DataLoader(
        train_set, batch_size=128, shuffle=train_shuffle, num_workers=args.num_workers
    )
    ...
    val_loader = DataLoader(
        val_set, batch_size=128, shuffle=False, num_workers=args.num_workers
    )
    ...
    test_loader = DataLoader(
        test_set, batch_size=128, shuffle=False, num_workers=args.num_workers
    )
    args.num_classes = 2
    return (train_loader, val_loader, test_loader)


def load_waterbirds_fail(args, train_shuffle=True, transform=None):
    """
    Default dataloader setup for Waterbirds
...
def visualize_waterbirds(
    ...
    plot_data_batch(
        [dataloader.dataset.__getitem__(ix)[0] for ix in all_data_indices],
...
def load_dataloaders(args, train_shuffle=True, val_correlation=None, transform=None):
...
def visualize_dataset(
    ...
    return visualize_waterbirds(
        dataloader, num_datapoints, title, args, save, save_id, ftype, target_type
    )
