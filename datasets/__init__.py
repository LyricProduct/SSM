...
def initialize_data(args):
    ...
    dataset_module = importlib.import_module(
        f"datasets.{dataset}"
    )  # モジュールを動的に読み込む。具体的にはdatasets/waterbirds.pyを読み込む
    load_dataloaders = getattr(
        dataset_module, "load_dataloaders"
    )  # waterbirds.pyのload_dataloadersを取得
    visualize_dataset = getattr(
        dataset_module, "visualize_dataset"
    )  # waterbirds.pyのvisualize_datasetを取得
...
def plot_data_batch(
    ...
    if save:
        try:
            fpath = join(args.image_path, f"{save_id}-{args.experiment_name}.{ftype}")
